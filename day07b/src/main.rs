...
const SPACE_AVAILABLE: u32 = 70000000;
const MIN_SPACE: u32 = 30000000;

fn find_directory_size(dir_map: &HashMap<String, u32>) -> u32 {
    let root_size = dir_map.get("/").expect("Could't get root size!");
    let free_space: u32 = SPACE_AVAILABLE - root_size;
    let space_needed: u32 = MIN_SPACE - free_space;
    ...
    for size in dir_map.values() {
        ...
        if (found_size == 0 && size >= space_needed) || (size >= space_needed && size < found_size)
        {
            found_size = size;
        }
    }
...
fn main() {
    ...
    let dir_size = find_directory_size(&directory_size_map);

    println!("Found directory size: {}", dir_size);
}
